...
export class RequestContextService {
	...
	private extractLanguageCode(req: Request): string {
		...
		if (req?.query?.[LANGUAGE_CODE_QUERY_NAME]) {
			return req.query[LANGUAGE_CODE_QUERY_NAME] as string;
		}

		return this.configService.defaultLanguageCode;
	}

	private extractCurrencyCode(req: Request): string {
		// 1. query param
		// 2. sales channel available currencies

		if (req?.query?.[CURRENCY_CODE_QUERY_NAME]) {
			const queryCurrencyCode = req.query[CURRENCY_CODE_QUERY_NAME] as string;
			// TODO: check if queryCurrencyCode is available in the sales channel
			// and throw error if not
			return queryCurrencyCode;
		}

		return ''; // this should return the defaultCurrencyCode of the channel
	}
}
